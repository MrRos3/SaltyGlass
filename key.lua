...
local CONFIG = {
    ...
    -- Roblox LocalScripts do not expose a real hardware ID.
    -- SaltyGlass uses a persistent install ID + fresh session ID.
    BindingProvider = nil,
...
local function parseExpiry(value)
    ...
    local y, m, d, h, min, s = value:match(
...
local function ensureFolder()
    if not persistence.available or not persistence.makefolder then
        return
    end
...
local function readSaved()
    if not CONFIG.RememberKey then
        return nil
    end

    if persistence.available then
        local ok, raw = pcall(function()
            if persistence.isfile and not persistence.isfile(CONFIG.SaveFile) then
                return nil
            end
...
local function normalizeAuth(data)
    ...
    if data.updateChannel == nil then
        data.updateChannel = "stable"
    end

    if type(data.featureFlags) ~= "table" then
        data.featureFlags = {}
    end
...
local function remoteValidate(key)
    ...
    if not requestFn then
        return false, {
            message = "No supported HTTP request function is available",
        }
    end

    if CONFIG.Remote.Url == "" then
        return false, {
            message = "Remote validator URL is empty",
        }
    end
    ...
    if not ok or type(response) ~= "table" then
        return false, {
            message = "Remote validation request failed",
        }
    end
    ...
    if status < 200 or status >= 300 then
        ...
        if type(body) == "string" then
            ...
            if decodeOk and type(data) == "table" and data.message then
...
local function validator(key)
    ...
    authState = normalizeAuth(result)
    ...
    if valid then
        writeSaved({
            ...
            role = authState.role,
            updateChannel = authState.updateChannel,
...
local function publishAuthToClient()
    local auth = normalizeAuth(authState)

    player:SetAttribute("SaltyRole", tostring(auth.role or "user"))
    player:SetAttribute(
        "SaltyUpdateChannel",
        tostring(auth.updateChannel or "stable")
    )
    player:SetAttribute(
        "SaltyFeatureFlags",
        HttpService:JSONEncode(auth.featureFlags or {})
    )

    pcall(function()
        if type(getgenv) == "function" then
            getgenv().SaltyAuth = auth
        end
    end)
...
local keySource = game:HttpGet(
    "https://raw.githubusercontent.com/MrRos3/SaltyGlass/main/key-system.lua?ui=stable"
)
local keyChunk, keyCompileError = loadstring(keySource)
assert(
    keyChunk,
    "SaltyGlass key-system.lua failed to compile: "
        .. tostring(keyCompileError)
)

local KeySystem = keyChunk()
assert(
    type(KeySystem) == "table"
        and type(KeySystem.Open) == "function",
...
local handle = KeySystem.Open({
    ...
    OnSuccess = function()
        ...
        local guiSource = game:HttpGet(
            "https://raw.githubusercontent.com/MrRos3/SaltyGlass/main/feature-pack.lua?framework=2.0.0"
        )
        local guiChunk, guiCompileError = loadstring(guiSource)

        assert(
            guiChunk,
            "SaltyGlass feature-pack.lua failed to compile: "
                .. tostring(guiCompileError)
        )
...
-- Keep the approved presentation even while key-system.lua stays reusable.
local gui = handle and handle:GetScreenGui()
if gui then
    for _, child in ipairs(gui:GetChildren()) do
        if child:IsA("Frame")
            and child.ZIndex == 2
            and child.Size.X.Offset == 522
            and child.Size.Y.Offset == 352 then
            child:Destroy()
        end
    end
    ...
    for _, item in ipairs(gui:GetDescendants()) do
        if item:IsA("ImageLabel")
            and item:GetAttribute("LucideName") == "shield" then
            shield = item
        elseif item:IsA("TextLabel")
            and string.find(
                item.Text,
                "ENCRYPTED SESSION",
                1,
...
if CONFIG.AutoVerifySaved
    and type(saved) == "table"
    and type(saved.key) == "string"
    and saved.key ~= ""
    and gui then
