...
if not ROLE_LEVELS[state.role] then
    state.role = "user"
end
if state.channel ~= "stable"
    and state.channel ~= "beta"
    and state.channel ~= "dev" then
...
local function playSound(id, volume)
    if screenGui:GetAttribute("UISoundsEnabled") == false then
        return
    end
...
local function click()
    playSound("rbxassetid://4307186075", 0.26)
end

local function hover()
    playSound("rbxassetid://408524543", 0.09)
end
...
local function notify(message, kind)
    ...
    while #state.notifications > 50 do
    ...
    tw(statusStroke, 0.12, {
        Transparency = 0.56,
        Color = kind == "error" and C.Danger or C.Edge,
    })
    tw(statusText, 0.12, { TextTransparency = 0 })

    task.delay(1.5, function()
        if token ~= statusToken or not status.Parent then
            return
        end
        ...
        task.delay(0.13, function()
            if token == statusToken and status.Parent then
                status.Visible = false
            end
        end)
    end)
...
local function makeButton(parent, options, order)
    ...
    connect(button.MouseEnter, function()
        hover()
        tw(button, 0.12, { BackgroundTransparency = 0.20 })
    end)

    connect(button.MouseLeave, function()
        tw(button, 0.12, { BackgroundTransparency = 0.38 })
    end)
...
local function makeToggle(parent, options, order)
    ...
    function control:Set(value, silent)
        ...
        tw(track, 0.14, {
            BackgroundColor3 = self.Value and C.Accent or C.Light,
...
local function makeSlider(parent, options, order)
    ...
    local hit = Instance.new("TextButton")
    hit.Position = UDim2.new(0, 8, 1, -28)
    ...
    local control = {
        Value = tonumber(options.Default) or minimum,
        Dragging = false,
    }

    local function formatValue(value)
        if step < 1 then
            return string.format("%.1f%s", value, options.Suffix or "")
        end
        return tostring(math.floor(value + 0.5)) .. (options.Suffix or "")
    end
    ...
    function control:Set(value, silent)
        ...
        local alpha = maximum > minimum
            and (self.Value - minimum) / (maximum - minimum)
            or 0
...
local function makeTextbox(parent, options, order)
    ...
    box.Size = UDim2.fromOffset(154, 34)
    box.BackgroundColor3 = C.Mid
    ...
    connect(box.FocusLost, function(enterPressed)
        outline.Color = C.Edge
        outline.Transparency = 0.70
        control:Set(box.Text, false)

        if enterPressed
            and type(options.OnEnter) == "function" then
            task.spawn(options.OnEnter, control.Value)
        end
    end)
...
local function deserializeValue(value)
    if type(value) == "table"
        and value.__type == "Color3"
        and type(value.hex) == "string" then
        return hexToColor(value.hex)
    end
    return value
end
        ...
        pcall(function()
            if type(makefolder) == "function"
                and (
                    ...
                    or not isfolder("SaltyGlass")
                ) then
                makefolder("SaltyGlass")
            end
        end)

        local ok = pcall(
            writefile,
            "SaltyGlass/profiles.json",
            raw
        )
...
local function loadProfiles()
    ...
    if type(readfile) == "function" then
        local ok, value = pcall(function()
            if type(isfile) == "function"
                and not isfile("SaltyGlass/profiles.json") then
                return nil
            end
            return readfile("SaltyGlass/profiles.json")
        end)
    ...
    if not raw then
        raw = player:GetAttribute("SaltyProfilesSession")
    end
...
local function snapshot()
    local result = {
        ...
        telemetry = state.telemetryEnabled,
        controls = {},
        flags = state.flags,
    }
    ...
    for id, control in pairs(state.controls) do
        if type(control.Get) == "function" then
            ...
            if ok then
                result.controls[id] = serializeValue(value)
            end
        end
    end
...
local function applySnapshot(data)
    ...
    if type(data.accent) == "string" then
        local color = hexToColor(data.accent)
        if color then
            API:SetAccent(color)
        end
    end
    ...
    if data.channel then
        API:SetUpdateChannel(data.channel)
    end

    if type(data.telemetry) == "boolean" then
        API:SetTelemetryEnabled(data.telemetry)
    end
    ...
    if type(data.flags) == "table" then
        ...
        player:SetAttribute(
            "SaltyFeatureFlags",
            HttpService:JSONEncode(state.flags)
        )
    end
...
function API:Notify(message, kind)
    notify(message, kind)
end

function API:GetNotificationHistory()
    local output = {}
    for i, item in ipairs(state.notifications) do
        output[i] = {
            text = item.text,
            kind = item.kind,
...
function API:ClearNotificationHistory()
    table.clear(state.notifications)
end
...
function API:HasRole(required)
    required = string.lower(tostring(required or "user"))
    return (ROLE_LEVELS[state.role] or 1)
        >= (ROLE_LEVELS[required] or 1)
end

function API:SetRole(role)
    role = string.lower(tostring(role or "user"))
    if not ROLE_LEVELS[role] then
        return false
    end

    state.role = role
    player:SetAttribute("SaltyRole", role)
    screenGui:SetAttribute("FeatureRole", role)
    return true
end

function API:SetFeatureFlag(name, enabled)
    name = tostring(name)
    state.flags[name] = enabled == true
...
function API:IsFeatureEnabled(name, defaultValue)
    local value = state.flags[tostring(name)]
    if value == nil then
...
function API:GetFeatureFlags()
    local copy = {}
    for name, value in pairs(state.flags) do
        copy[name] = value
    end
    return copy
end

function API:GetUpdateChannel()
    return state.channel
end

function API:SetUpdateChannel(channel)
    channel = string.lower(tostring(channel))

    if channel ~= "stable"
        and channel ~= "beta"
        and channel ~= "dev" then
        return false
    end

    state.channel = channel
    player:SetAttribute("SaltyUpdateChannel", channel)
    screenGui:SetAttribute("UpdateChannel", channel)
    return true
end

function API:SetTelemetryEnabled(enabled)
    state.telemetryEnabled = enabled == true
end

function API:IsTelemetryEnabled()
    return state.telemetryEnabled
end

function API:SetTelemetrySink(callback)
    state.telemetrySink = type(callback) == "function"
        and callback
        or nil
end

function API:EmitTelemetry(eventName, data)
    if not state.telemetryEnabled
        or type(state.telemetrySink) ~= "function" then
        return false
    end
    ...
    for _, item in ipairs(screenGui:GetDescendants()) do
        pcall(function()
            if (
                ...
                or item:IsA("TextBox")
            ) and item.BackgroundColor3 == previous then
            ...
            if (
                ...
                or item:IsA("TextButton")
                or item:IsA("TextBox")
            ) and item.TextColor3 == previous then
                item.TextColor3 = color
            end

            if (
                item:IsA("ImageLabel")
                or item:IsA("ImageButton")
            ) and item.ImageColor3 == previous then
                item.ImageColor3 = color
            end

            if item:IsA("UIStroke")
                and item.Color == previous then
                item.Color = color
            end
...
function API:GetAccent()
    return C.Accent
end

function API:GetAccentHex()
    return colorToHex(C.Accent)
end

function API:SetAccentHex(value)
...
function API:LoadProfile(name)
    local data = state.profiles[tostring(name or "Default")]
    if type(data) ~= "table" then
        return false
    end
...
function API:DeleteProfile(name)
    state.profiles[tostring(name)] = nil
    saveProfiles()
    return true
end

function API:ListProfiles()
    local output = {}
    for name in pairs(state.profiles) do
...
function API:BackupConfig()
    state.backup = snapshot()
    return state.backup
end

function API:RestoreBackup()
    return applySnapshot(state.backup)
end

function API:RegisterCommand(name, callback, options)
    name = string.lower(tostring(name or ""))
    assert(name ~= "", "Command requires a name")
    assert(type(callback) == "function", "Command requires callback")
    ...
    state.commands[name] = {
        ...
        role = string.lower(tostring(options.Role or "user")),
    }
...
function API:ExecuteCommand(name, ...)
    ...
    if not self:HasRole(command.role) then
    ...
    if not ok then
        warn("[SaltyGlass] command failed:", name, result)
...
function API:RunCommandLine(line)
    local parts = {}
    ...
    return self:ExecuteCommand(name, table.unpack(parts))
end
...
function API:RegisterModule(name, module)
    ...
    assert(name ~= "", "Module requires a name")

    if type(module) == "function" then
        local ok, result = pcall(module, API)
        if not ok then
            error(
                "SaltyGlass module failed: "
                    .. tostring(result),
                2
            )
        end
        module = result or module
    elseif type(module) == "table"
        and type(module.Init) == "function" then
        local ok, err = pcall(module.Init, module, API)
        if not ok then
            error(
                "SaltyGlass module Init failed: "
                    .. tostring(err),
                2
            )
        end
...
function API:LoadModule(sourceOrUrl, name)
    assert(
        type(sourceOrUrl) == "string",
        "LoadModule expects source or URL"
    )

    local source = sourceOrUrl

    if sourceOrUrl:match("^https?://") then
        source = game:HttpGet(sourceOrUrl)
    end
...
function API:GetModule(name)
    return state.modules[tostring(name)]
end
...
function Section:AddToggle(options)
    ...
    if options.Id then
        state.controls[options.Id] = control
    end
...
local function buildFeature(definition)
    ...
    if state.built[id] then
        return true
    end

    if definition.Role
        and not API:HasRole(definition.Role) then
        return false
    end

    if definition.Flag
        and not API:IsFeatureEnabled(
            definition.Flag,
            true
        ) then
        return false
    ...
    local parent = bodies[
        tostring(definition.Tab or "Visuals")
    ]
    ...
    local order = tonumber(definition.Order) or 800
    ...
    if not ok then
        warn(
            "[SaltyGlass] feature build failed:",
            id,
            err
        )
        return false
...
function API:RegisterFeature(definition)
    assert(
    ...
    definition.Id = id
    state.features[id] = definition
    buildFeature(definition)
    return definition
end

function API:GetRegisteredFeatures()
    local output = {}

    for id, definition in pairs(state.features) do
...
function API:GetControl(id)
    return state.controls[id]
end

-- Built-in commands for future modules / scripts.
API:RegisterCommand("notify", function(self, ...)
    self:Notify(table.concat({ ... }, " "))
    return true
end, {
    Description = "Show a Salty notification.",
})

API:RegisterCommand("channel", function(self, value)
    if value == nil then
        return self:GetUpdateChannel()
    end

    if not self:SetUpdateChannel(value) then
        error("channel must be stable, beta, or dev")
    end

    self:Notify(
        "Update channel • "
            .. string.upper(self:GetUpdateChannel())
    )

    return self:GetUpdateChannel()
end, {
    Description = "Get/set update channel.",
})

API:RegisterCommand("flag", function(self, name, value)
    if not name then
        error("flag requires a name")
    end

    if value == nil then
        return self:IsFeatureEnabled(name, false)
    end
    ...
    Description = "Get/set feature flag.",
    Role = "beta",
})

API:RegisterCommand("profile", function(self, action, name)
    action = string.lower(tostring(action or "list"))
    name = tostring(name or state.profileName)

    if action == "save" then
        self:SaveProfile(name)
        return "saved " .. name
    elseif action == "load" then
        if not self:LoadProfile(name) then
    ...
    error("profile action must be save/load/delete/list")
end, {
...
if bodies.Visuals then
    ...
    local colorState = {
        r = math.floor(C.Accent.R * 255 + 0.5),
        g = math.floor(C.Accent.G * 255 + 0.5),
        b = math.floor(C.Accent.B * 255 + 0.5),
        syncing = false,
    }

    local previewCard = card(
        bodies.Visuals,
        62,
        901
    )
    ...
    preview.BackgroundColor3 = C.Accent
    ...
    hex.AnchorPoint = Vector2.new(1, 0.5)
    hex.Position = UDim2.new(
        1,
        -14,
        0.5,
        0
    )
    hex.Size = UDim2.fromOffset(146, 34)
    hex.BackgroundColor3 = C.Mid
    hex.BackgroundTransparency = 0.28
    hex.BorderSizePixel = 0
    hex.ClearTextOnFocus = false
    hex.Font = Enum.Font.GothamBold
    hex.Text = colorToHex(C.Accent)
    hex.TextColor3 = C.Text
    hex.TextSize = 10
    hex.TextXAlignment = Enum.TextXAlignment.Center
    hex.ZIndex = 7
    hex.Parent = previewCard
    round(hex, 9)
    local hexStroke = stroke(hex, 0.68)
    ...
    local function applyColor(color, announce)
        colorState.syncing = true

        colorState.r = math.floor(
            color.R * 255 + 0.5
        )
        colorState.g = math.floor(
            color.G * 255 + 0.5
        )
        colorState.b = math.floor(
            color.B * 255 + 0.5
        )
        ...
        if red then
            red:Set(colorState.r, true)
            green:Set(colorState.g, true)
            blue:Set(colorState.b, true)
        end

        API:SetAccent(color)
        colorState.syncing = false
    ...
    red = makeSlider(
        ...
        {
            Name = "Red",
            Description = "Accent red channel.",
            Min = 0,
            Max = 255,
            Default = colorState.r,
            Increment = 1,
    ...
    green = makeSlider(
        ...
        {
            Name = "Green",
            Description = "Accent green channel.",
            Min = 0,
            Max = 255,
            Default = colorState.g,
            Increment = 1,
            Callback = function(value)
                colorState.g = value
                applyRgb()
    ...
    blue = makeSlider(
        ...
        {
            Name = "Blue",
            Description = "Accent blue channel.",
            Min = 0,
            Max = 255,
            Default = colorState.b,
            Increment = 1,
            Callback = function(value)
                colorState.b = value
                applyRgb()
    ...
    state.controls["appearance.red"] = red
    state.controls["appearance.green"] = green
    state.controls["appearance.blue"] = blue

    connect(hex.Focused, function()
        hover()
        hexStroke.Color = C.Accent
        hexStroke.Transparency = 0.34
    end)

    connect(hex.FocusLost, function(enterPressed)
        hexStroke.Color = C.Edge
        hexStroke.Transparency = 0.68

        local color = hexToColor(hex.Text)

        if color then
            applyColor(color, true)
        else
            hex.Text = colorToHex(C.Accent)
            notify("Invalid HEX color", "error")
        end
    end)
    ...
    makeButton(
        ...
        {
            Name = "Reset Accent",
            Description = "Restore the original Salty violet accent.",
            ButtonText = "RESET",
            Callback = function()
                applyColor(
                    Color3.fromRGB(
...
if bodies.Settings then
    ...
    textPair(
        roleCard,
        "Permission Role",
        "Role supplied by key validation.",
        130
    )

    local roleLabel = Instance.new("TextLabel")
    roleLabel.AnchorPoint = Vector2.new(1, 0.5)
    roleLabel.Position = UDim2.new(
        1,
        -14,
        0.5,
        0
    )
    roleLabel.Size = UDim2.fromOffset(104, 28)
    roleLabel.BackgroundColor3 = C.Light
    roleLabel.BackgroundTransparency = 0.46
    roleLabel.BorderSizePixel = 0
    roleLabel.Font = Enum.Font.GothamBold
    roleLabel.Text = string.upper(state.role)
    roleLabel.TextColor3 = C.Accent
    roleLabel.TextSize = 9
    roleLabel.ZIndex = 6
    roleLabel.Parent = roleCard
    round(roleLabel, 9)
    stroke(roleLabel, 0.72)

    local channelButton
    channelButton = makeButton(
        bodies.Settings,
        {
            Name = "Update Channel",
            Description = "Cycle stable, beta, and dev.",
            ButtonText = string.upper(state.channel),
            Callback = function()
                local nextChannel = ({
                    stable = "beta",
                    beta = "dev",
                    dev = "stable",
                })[state.channel] or "stable"

                API:SetUpdateChannel(nextChannel)
                channelButton.Text = string.upper(
                    nextChannel
                )

                notify(
                    "Update channel • "
                        .. string.upper(nextChannel)
                )
            end,
        },
        902
    ...
    local profileName = makeTextbox(
        bodies.Settings,
        {
            Name = "Profile Name",
            Description = "Name used by save/load.",
            Placeholder = "Default",
    ...
    makeButton(
        ...
        {
            Name = "Save Profile",
            Description = "Save framework/custom settings.",
            ButtonText = "SAVE",
            Callback = function()
                API:SaveProfile(state.profileName)
    ...
    makeButton(
        ...
        {
            Name = "Delete Profile",
            Description = "Remove the selected saved profile.",
            ButtonText = "DELETE",
            Callback = function()
                API:DeleteProfile(
                    state.profileName
    ...
    local telemetry = makeToggle(
        bodies.Settings,
        {
            Name = "Telemetry Hook",
            Description = "Opt-in callback only. No built-in endpoint.",
            Default = false,
            Callback = function(enabled)
                API:SetTelemetryEnabled(enabled)

                notify(
                    enabled
                        and "Telemetry hook enabled"
                        or "Telemetry hook disabled"
                )
            end,
        },
        907
    ...
    makeButton(
        ...
        {
            Name = "Restore Backup",
            Description = "Restore the latest recovery snapshot.",
            ButtonText = "RESTORE",
            Callback = function()
                if API:RestoreBackup() then
                    notify(
                        "Backup restored",
                        "success"
                    )
    ...
    makeButton(
        bodies.Settings,
        {
            ...
            Description = "Print recent Salty messages.",
            ButtonText = "HISTORY",
            Callback = function()
                print(
    ...
    makeButton(
        ...
        {
            Name = "Feature Flags",
            Description = "Print active remote/local feature flags.",
            ButtonText = "FLAGS",
            Callback = function()
                print("[SaltyGlass] Feature Flags")

                local flags = API:GetFeatureFlags()
                local names = {}
    ...
    local commandInput = makeTextbox(
        bodies.Settings,
        {
            Name = "Command",
            Description = "Run registered Salty commands.",
    ...
    makeButton(
        ...
        {
            Name = "Run Command",
            Description = "Execute through the permission-aware bridge.",
            ButtonText = "RUN",
            Callback = function()
                local ok, result = API:RunCommandLine(
                    commandInput:Get()
                )
    ...
    makeButton(
        ...
        {
            Name = "List Commands",
            Description = "Print registered framework commands.",
            ButtonText = "LIST",
            Callback = function()
                print("[SaltyGlass] Commands")
...
connect(screenGui.Destroying, function()
    ...
    if _G.SaltyGlass == API then
        _G.SaltyGlass = nil
    end

    pcall(function()
        if type(getgenv) == "function"
            and getgenv().SaltyGlass == API then
            getgenv().SaltyGlass = nil
        end
    end)
end)

notify(
    "Framework 2.1 + RGB/HEX ready",
    "success"
)
